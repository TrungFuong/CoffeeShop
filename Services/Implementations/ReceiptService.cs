...
namespace CoffeeShop.Services.Implementations
{
    public class ReceiptService : IReceiptService
    {
        ...
        {
            //var customer = await _unitOfWork.CustomerRepository.GetAsync(r => r.CustomerId == receiptRequestDTO.CustomerId);
        ...
        //public async Task<IEnumerable<ReceiptResponseDTO>> GetAllReceiptsAsync(int page, string? search, string? sortOrder, string? sortBy = "receiptDate", string includeProperties = "")
        //{
        ...
        private async Task<Expression<Func<Receipt, bool>>>? GetFilterQuery(string? search)
        {
            ...
            if (!string.IsNullOrEmpty(search))
            {
                var searchCondition =
                    Expression.Call(
                        //REMEMBER TO FINISH
                        Expression.Property(parameter, nameof(Receipt)),
